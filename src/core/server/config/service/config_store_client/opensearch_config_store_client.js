...
class OpenSearchConfigStoreClient {
  ...
  async createDynamicConfigIndex() {
    const existsAliasResponse = await _classPrivateFieldGet2(this, _openSearchClient).indices.existsAlias({
    ...
    if (!existsAliasResponse.body) {
      ...
      if (latestVersion < 1) {
        await _classPrivateFieldGet2(this, _openSearchClient).indices.create({
        ...
        await _classPrivateFieldGet2(this, _openSearchClient).indices.updateAliases({
          body: {
            actions: [{
              add: {
      ...
      const results = await _classPrivateFieldGet2(this, _openSearchClient).indices.getAlias({
        name: _constants.DYNAMIC_APP_CONFIG_ALIAS
      });
  ...
  async getConfig(namespace, options) {
    if (_classPrivateFieldGet2(this, _cache).has(namespace)) {
      return _classPrivateFieldGet2(this, _cache).get(namespace);
    }
    const result = (await this.searchConfigsRequest([namespace])).body.hits.hits;
    if (result.length <= 0) {
      _classPrivateFieldGet2(this, _cache).set(namespace, undefined);
      return undefined;
    }
  ...
  async bulkGetConfigs(namespaces, options) {
    ...
    const configsToQuery = namespaces.filter(namespace => {
      const isCached = _classPrivateFieldGet2(this, _cache).has(namespace);
      const config = _classPrivateFieldGet2(this, _cache).get(namespace);
      if (config) {
    ...
    nonExistentConfigs.forEach(name => {
      _classPrivateFieldGet2(this, _cache).set(name, undefined);
    });
    return results;
  }
  async listConfigs(options) {
    // Cannot get from cache since config keys can be missing
    const configs = await _classPrivateFieldGet2(this, _openSearchClient).search({
  ...
  clearCache() {
    _classPrivateFieldGet2(this, _cache).clear();
  }
  ...
  setCacheFromSearch(config) {
    ...
    _classPrivateFieldGet2(this, _cache).set(config_name, config_blob);
  }
   ...
   */
  setCache(config) {
    _classPrivateFieldGet2(this, _cache).set(config.config_name, config.config_blob);
  }
  ...
  async createConfigsRequest(configMap) {
    ...
    return await _classPrivateFieldGet2(this, _openSearchClient).bulk({
      index: _constants.DYNAMIC_APP_CONFIG_ALIAS,
      body: bulkConfigs
    });
  }
  ...
  async deleteConfigsRequest(namespaces) {
    namespaces.forEach(name => _classPrivateFieldGet2(this, _cache).delete(name));
    return await _classPrivateFieldGet2(this, _openSearchClient).deleteByQuery({
      index: _constants.DYNAMIC_APP_CONFIG_ALIAS,
      body: {
        query: {
          bool: {
  ...
  async searchConfigsRequest(namespaces, excludeConfigBlob = false) {
    return await _classPrivateFieldGet2(this, _openSearchClient).search({
      ...(excludeConfigBlob && {
        _source: ['config_name']
      }),
      index: _constants.DYNAMIC_APP_CONFIG_ALIAS,
  ...
  async searchLatestConfigIndex() {
    const configIndices = await _classPrivateFieldGet2(this, _openSearchClient).cat.indices({
