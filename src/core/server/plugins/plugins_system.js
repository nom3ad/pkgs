...
class PluginsSystem {
  constructor(coreContext) {
    ...
    _defineProperty(this, "crossCompatibilityService", void 0);
    ...
    this.crossCompatibilityService = new _cross_compatibility.CrossCompatibilityService(coreContext);
  }
  addPlugin(plugin) {
  ...
  async healthCheckOpenSearchPlugins(deps) {
    ...
    const opensearchInstalledPlugins = await this.crossCompatibilityService.getOpenSearchPlugins(deps.opensearch);
    for (const pluginName of this.satupPlugins) {
      this.log.debug(`For plugin "${pluginName}"...`);
      const plugin = this.plugins.get(pluginName);
      this.crossCompatibilityService.checkPluginVersionCompatibility(plugin.requiredEnginePlugins, opensearchInstalledPlugins, pluginName);
    }
  }
  async stopPlugins() {
