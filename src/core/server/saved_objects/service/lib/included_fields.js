...
function includedFields(type = '*', fields) {
  ...
  }, []).concat('namespace').concat('namespaces').concat('type').concat('references').concat('migrationVersion').concat('updated_at').concat('originId').concat('workspaces').concat('permissions').concat(fields); // v5 compatibility
}