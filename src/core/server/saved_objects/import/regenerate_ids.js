 ...
 */
const regenerateIds = (objects, dataSourceId) => {
  const importIdMap = objects.reduce((acc, object) => {
    return object.type === 'data-source' ? acc : acc.set(`${object.type}:${object.id}`, {
      id: dataSourceId ? `${dataSourceId}_${(0, _uuid.v4)()}` : (0, _uuid.v4)(),
      omitOriginId: true
    });
  }, new Map());
  return importIdMap;
