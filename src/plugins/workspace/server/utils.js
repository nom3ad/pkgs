...
exports.updateDashboardAdminStateForRequest = exports.translatePermissionsToRole = exports.transferCurrentUserInPermissions = exports.getDataSourcesList = exports.generateRandomId = exports.checkAndSetDefaultDataSource = void 0;
...
const updateDashboardAdminStateForRequest = (request, groups, users, configGroups, configUsers) => {
  ...
  if (!groups.length && !users.length) {
    return (0, _utils.updateWorkspaceState)(request, {
      isDashboardAdmin: true
    });
  }
  // If user config contains wildcard characters '*', login defaults to OSD Admin
  if (configUsers.includes(_constants.OSD_ADMIN_WILDCARD_MATCH_ALL)) {
    return (0, _utils.updateWorkspaceState)(request, {
      isDashboardAdmin: true
    });
  }
...
const getDataSourcesList = (client, workspaces) => {
  return client.find({
    type: _constants.WORKSPACE_DATA_SOURCE_AND_CONNECTION_OBJECT_TYPES,
...
const translatePermissionsToRole = (isPermissionControlEnabled, permissions, principals) => {
  ...
  if (isPermissionControlEnabled && permissions) {
    ...
    [_server.WorkspacePermissionMode.Write, _server.WorkspacePermissionMode.LibraryWrite, _server.WorkspacePermissionMode.LibraryRead, _server.WorkspacePermissionMode.Read].forEach(mode => {
    ...
    if (modes.includes(_server.WorkspacePermissionMode.LibraryWrite) && modes.includes(_server.WorkspacePermissionMode.Write)) {
      permissionMode = _server.PermissionModeId.Owner;
    } else if (modes.includes(_server.WorkspacePermissionMode.LibraryWrite)) {
