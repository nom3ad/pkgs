...
function registerValueSuggestionsRoute(router, config$) {
  router.post({
    ...
    try {
      let result;
      if (dataSourceId) {
        const dataSourceClient = await context.dataSource.opensearch.legacy.getClient(dataSourceId);
        result = await dataSourceClient.callAPI('search', {
          index,
          body
        }, {
          signal
        ...
        result = await client.callAsCurrentUser('search', {
          index,
          body
        }, {
          signal
